...
| 33 | **`ruff` and `mypy --strict`** | `ruff` is linter and import-sorter in one; `mypy --strict` rejects an unannotated function, an untyped call, and a value returned as `Any`. Both configure from `pyproject.toml`, so there are no extra dotfiles. The useful part is not the style pass — it is that `--strict` makes you write down invariants you were carrying in your head. | A wrong VRAM number does not crash: it prints, looks plausible, and sends someone to buy a bigger GPU. `--strict` found three places where a type was wider than the invariant behind it — chiefly `lora_rank: int | None` being read as an `int` all through `advisor.py`, which is true only because `advise()` refuses a full-fine-tuning config at the door. That reasoning is now a guard instead of an assumption. SPEC §3.10 has the configuration and the full first-pass list. |
